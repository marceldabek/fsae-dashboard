...
export const TableRowActionsDropdown = ({ onEdit, onCopyUsername, onDelete }: {
    onEdit?: () => void;
    onCopyUsername?: () => void;
    onDelete?: () => void;
}) => (
    <Dropdown.Root>
        ...
        <Dropdown.Popover className="w-min">
            <Dropdown.Menu>
                <Dropdown.Item icon={Edit01} onAction={onEdit as any}>
                    <span className="pr-4">Edit</span>
                </Dropdown.Item>
                <Dropdown.Item icon={Copy01} onAction={onCopyUsername as any}>
                    <span className="pr-4">Copy username</span>
                </Dropdown.Item>
                <Dropdown.Item icon={Trash01} onAction={onDelete as any}>
...
const TableCardRoot = ({ children, className, size = "md", ...props }: HTMLAttributes<HTMLDivElement> & { size?: "sm" | "md" }) => {
    return (
        <TableContext.Provider value={{ size }}>
            <div {...props} className={cx("overflow-hidden rounded-xl bg-secondary shadow-xs ring-1 ring-secondary_alt", className)}>
                {children}
            </div>
...
const TableHeader = <T extends object>({ columns, children, bordered = true, className, ...props }: TableHeaderProps<T>) => {
    ...
    <AriaTableHeader
            ...
            className={(state) =>
                cx(
            "relative bg-secondary/80",
                    size === "sm" ? "h-9" : "h-11",

                    // Row border—using an "after" pseudo-element to avoid the border taking up space.
                    bordered &&
                        "[&>tr>th]:after:pointer-events-none [&>tr>th]:after:absolute [&>tr>th]:after:inset-x-0 [&>tr>th]:after:bottom-0 [&>tr>th]:after:h-px [&>tr>th]:after:bg-secondary [&>tr>th]:focus-visible:after:bg-transparent",
        ...
        <AriaColumn className={cx("relative py-2 pr-0 pl-4", size === "sm" ? "w-9 md:pl-5" : "w-11 md:pl-6")}>
            ...
            <div className="flex items-start opacity-90 group-data-[has-selection=false]:opacity-70 transition-opacity" data-has-selection>
...
const TableRow = <T extends object>({ columns, children, className, highlightSelectedRow = true, ...props }: TableRowProps<T>) => {
    ...
    return (
        <AriaRow
            ...
            className={(state) =>
                cx(
                    "relative outline-focus-ring transition-colors after:pointer-events-none bg-secondary hover:bg-secondary/70 focus-visible:outline-2 focus-visible:-outline-offset-2",
                    size === "sm" ? "h-14" : "h-18",
                    highlightSelectedRow && "selected:bg-primary/80",
