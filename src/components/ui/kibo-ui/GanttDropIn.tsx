...
export type GanttFeature = {
  ...
  name: string;
  startAt: Date;
  endAt: Date | null; // nullable throughout the file
  status: GanttStatus;
...
const getDateByMousePosition = (context: GanttContextProps, mouseX: number) => {
  const timelineStartYear = context.timelineData[0]?.year ?? new Date().getFullYear();
...
const createInitialTimelineData = (today: Date) => {
  ...
  data.push(
    { year: today.getFullYear() - 1, quarters: new Array(4).fill(null) as any },
    { year: today.getFullYear(), quarters: new Array(4).fill(null) as any },
    { year: today.getFullYear() + 1, quarters: new Array(4).fill(null) as any }
  );
...
export const GanttSidebarHeader: FC = () => (
  <div
    className="sticky top-0 z-20 flex shrink-0 items-end justify-between gap-2.5 border-border/50 border-b bg-backdrop/90 p-2.5 font-medium text-muted-foreground text-xs backdrop-blur-sm"
    style={{ height: 'var(--gantt-header-height)' }}
  >
...
export const GanttFeatureRow: FC<GanttFeatureRowProps> = ({
  ...
  type FeatureWithSubRow = GanttFeature & { subRow: number; };
  const featureWithPositions: FeatureWithSubRow[] = [];
  const subRowEndTimes: Date[] = []; // Track when each sub-row becomes free

  for (const feature of sortedFeatures) {
    const end = feature.endAt ?? addDays(feature.startAt, 1);
    let subRow = 0;
    ...
    if (subRow === subRowEndTimes.length) {
      subRowEndTimes.push(end);
    } else {
      subRowEndTimes[subRow] = end;
    }
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  let columnWidth = 50;
  if (range === 'monthly') columnWidth = 150;
  else if (range === 'quarterly') columnWidth = 100;
  ...
  return (
    ...
    >
      <div
        className={cn(
          'gantt relative grid h-full w-full select-none overflow-auto rounded-none bg-transparent',
          range,
          className
        )}
        ref={scrollRef}
