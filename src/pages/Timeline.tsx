...
export default function Timeline() {
  ...
  if (loading) return <div className="text-sm text-uconn-muted">Loading…</div>;

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold">Timeline</h1>
      <p className="text-sm text-uconn-muted">Lightweight scaffold to manage projects and tasks in a time-ordered list. Future: full calendar or external integration.</p>

      <div className="space-y-4">
        {projects.map(p => (
          <section key={p.id} className="border border-uconn-border rounded-md bg-black/10 p-4">
            <h2 className="font-medium text-lg">{p.name}</h2>
            {p.description && <p className="text-sm text-uconn-muted mb-2">{p.description}</p>}
